...
def test_full_tof(json_path, outdir,system):
    ...
    mlem_full_without_data = nef.io.json_load(nef.functions.MlemFullTof, json_path)
    if system == '8panel':
    ...
    for atten in opt_atten:
        for psf in opt_psf:
                ...
                nef.save(img, outdir + f'/recon_image{out_append}_tof.hdf5')
                ...
                nef.doc_gen(mlem_full_without_data, img, outdir,
                            outdir + f'/recon_doc{out_append}_tof.md')
                pypandoc.convert_file(outdir + f'/recon_doc{out_append}_tof.md', 'pdf',
                                      outputfile = outdir + f'/recon_doc{out_append}_tof.md' + '.pdf')
...
@click.option('--outdir', '-o', default = None)
@click.option('-s', '--system', default = 'mct')
def test_with_scatter_tof(json_path, outdir,system):
    ...
    for atten in opt_atten:
        for psf in opt_psf:               
            ...
            print('*************************************************')
            print(out_append)
            img = nef.load(nef.Image,outdir + f'/recon_image{out_append}_tof.hdf5')          
