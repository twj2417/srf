...
@cuda.jit(device = True)
def block_id(event, nb_crystals_per_ring, block_grid_y, num_block):
    theta_event = get_block_theta(event)
    if event[1] < 0:
        theta_event = 360 - theta_event
    fixed_theta = (theta_event + 180 / nb_crystals_per_ring * block_grid_y) % 360
    return int(fixed_theta / 360 * num_block)


@cuda.jit(device = True)
def crystal_id(nb_crystals_per_ring, nb_blocks_per_ring, inner_radius, outer_radius, grid_block,
               size_block, event):
    id_block = block_id(event, nb_crystals_per_ring, grid_block[1], nb_blocks_per_ring)
    ...
    dist = (block_center_x - event[0]) * math.cos(theta_block) + (
            block_center_y - event[1]) * math.sin(theta_block)
    ...
    id_crystal = int(
        norm(pro_point_x - edge_block_x, pro_point_y - edge_block_y) / size_block[1] * grid_block[
            1])
...
@cuda.jit(device = True)
def position2crystalid(nb_crystals_per_ring, nb_blocks_per_ring, nb_rings, inner_radius,
                       outer_radius, axial_length, grid_block,
                       size_block, event):
    ring_id1 = ring_id(nb_rings, axial_length, grid_block, size_block, event[2])
    crystal_id1 = crystal_id(nb_crystals_per_ring, nb_blocks_per_ring, inner_radius, outer_radius,
                             grid_block, size_block, event[:2])
    ring_id2 = ring_id(nb_rings, axial_length, grid_block, size_block, event[5])
    crystal_id2 = crystal_id(nb_crystals_per_ring, nb_blocks_per_ring, inner_radius, outer_radius,
                             grid_block, size_block, event[3:5])
    return (ring_id1 * nb_crystals_per_ring + crystal_id1), (
            ring_id2 * nb_crystals_per_ring + crystal_id2)
...
@cuda.jit
def crystalid(nb_crystals_per_ring, nb_blocks_per_ring, nb_rings, inner_radius, outer_radius,
              axial_length, grid_block, size_block, events, final_id):
    c, j = cuda.grid(2)
    i = 512 * 16 * c + j
    if i < events.shape[0]:
        id1, id2 = position2crystalid(nb_crystals_per_ring, nb_blocks_per_ring, nb_rings,
                                      inner_radius, outer_radius, axial_length,
...
@jit(nopython = True)
def cal_sinogram(list_mode_data, nb_rings, grid_block_z, nb_crystals_per_ring):
    total_num_crystal = nb_rings * grid_block_z * nb_crystals_per_ring
...
def lm2sino(listmode, scanner):
    ...
    crystalid[(512, 512), (16, 16)](scanner.nb_blocks_per_ring * scanner.blocks.shape[1],
                                    scanner.nb_blocks_per_ring, scanner.nb_rings,
                                    scanner.inner_radius,
                                    scanner.outer_radius, scanner.axial_length,
                                    np.array(scanner.blocks.shape, dtype = np.int32),
                                    np.array(scanner.blocks.size, dtype = np.float32),
                                    listmode_pos, listmode_id)
    listmode_data = np.hstack((listmode_id, listmode.data.reshape(-1, 1)))
    return cal_sinogram(listmode_data, scanner.nb_rings, scanner.blocks.shape[2],
                        scanner.nb_blocks_per_ring * scanner.blocks.shape[1])
