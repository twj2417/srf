...
def fkn(A, S, B, u_map, size, shape):
    ...
    sigma = math.pi * (40.0 / 9.0 - 3.0 * math.log(3.0))
    return differential_value / sigma * u_map[grid_S0, grid_S1, grid_S2]

@cuda.jit(device = True)
def fkn_new(A,S,B,u_map,size,shape):
    cos_asb = math.cos(get_scatter_cos_theta(A, S, B))
    grid_S0 = int((S[0] / size[0] + 0.5) * shape[0])
    grid_S1 = int((S[1] / size[1] + 0.5) * shape[1])
    grid_S2 = int((S[2] / size[2] + 0.5) * shape[2])
    differential_value = 1.0 / (2.0 - cos_asb) ** 2 * ((1.0 + cos_asb ** 2) / 2.0
                                                        + (1.0 - cos_asb) ** 2 / 2.0 / (
                                                                    2.0 - cos_asb))
    total_cross = 1000*(u_map[grid_S0, grid_S1, grid_S2]-0.0092)/0.0092*9.57*10**-5+9.5784*10**-2
    differential_cross = total_cross/(math.pi*(40.0 / 9.0 - 3.0 * math.log(3.0)))*differential_value
    return differential_cross
...
def get_scatter_cos_theta(A, S, B):
    ...
    return int(math.acos(-(dis_as ** 2 + dis_sb ** 2 - dis_ab ** 2) / (2 * dis_as * dis_sb))/math.pi*180)/180*math.pi
