...
__device__ float fkn(const float x1, const float y1, const float z1,
                     const float x2, const float y2, const float z2,
                     ...
                     const float *umap)
{
    ...
    {
        float Z = 10;
        float sigma = PI * Z * (40.0 / 9.0 - 3.0 * logf(3.0));
        float differential_value = 1 / (2 - cos_asb) / (2 - cos_asb) * ((1.0 + cos_asb * cos_asb) / 2 + (1.0 - cos_asb) * (1.0 - cos_asb) / 2 / (2 - cos_asb));
        return differential_value / sigma * umap[ix + nx * iy + nx * ny * iz];
    }
}
    ...
    for (int ix = 0; ix < nx; ix += smp_x)
    {
        for (int iy = 0; iy < ny; iy += smp_y)
        {
            for (int iz = 0; iz < nz; iz += smp_z)
            {
                ...
                float fkn_value = fkn(x1, y1, z1,
                                      x2, y2, z2, xp, yp, zp, ix, iy, iz,
                                      nx, ny, nz, cos_theta, umap);
                                 ...
                                 float *lor_scales)
{
    ...
    // lor_scales[0] = 4.0f * PI * length_ab * length_ab / epsilon_ab / epsilon_ab / cos_phi_ab / cos_phi_ba / crystal_area / crystal_area;
    // if (area_ab * area_ba < 0.000001)
    // {lor_scales[0] = 4.0f * PI * length_ab * length_ab / epsilon_ab / epsilon_ab / 0.000001; return;}
...
// __device__ void test_lor_scatter(
//     const float *umap,
//     const int tid,
//     const int nx, const int ny, const int nz,
//     float *vproj)
...
//     int step = blockDim.x * gridDim.x;
//     // int jid = threadIdx.x;
//     for (int tid = blockIdx.x * blockDim.x + threadIdx.x; tid < (nx * ny * nz + step); tid += step)
//     {
//         if (tid >= nx * ny * nz)
//         {
//             return;
//         }
//         test_lor_scatter(umap,
//                          tid,
//                          nx, ny, nz,
//                          vproj + tid);
//     }
